...
export class AutonomousTMSController {
  ...
  public async processEvent(eventType: string, eventData: any): Promise<any> {
    if (!this.isRunning) {
  ...
  private startShipmentProcessing(): void {
    setInterval(async () => {
  ...
  private startCustomerServiceProcessing(): void {
    setInterval(async () => {
      if (!this.isRunning) return;
  ...
  private startFinancialProcessing(): void {
    setInterval(async () => {
      if (!this.isRunning) return;
  ...
  private determinePriority(eventType: string, eventData: any): 'low' | 'medium' | 'high' | 'critical' {
    ...
    return (priorityMap[eventType] as any) || 'medium';
  }

  private mapEventType(eventType: string): 'shipment' | 'customer_service' | 'financial' | 'analytics' {
    // Map event types to decision categories
    if (eventType.includes('shipment')) return 'shipment';
    if (eventType.includes('customer')) return 'customer_service';
    if (eventType.includes('financial')) return 'financial';
    return 'analytics';
  }

  private async logEvent(eventType: string, eventData: any, decision: any): Promise<void> {
    await this.supabase
      .from('system_events')
      .insert({
