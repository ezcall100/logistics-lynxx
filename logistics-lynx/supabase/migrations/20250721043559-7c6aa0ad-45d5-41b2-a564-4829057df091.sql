-- Create carrier-driver relationship and authentication system (Fixed with correct column references)

-- Add carrier relationship fields to profiles
...
-- Create driver invitations table
...
CREATE POLICY "Carrier admins can manage driver invitations" 
ON public.driver_invitations 
FOR ALL 
USING (
  invited_by = auth.uid() OR
  carrier_id IN (
    SELECT p.company::uuid FROM public.profiles p
    WHERE p.user_id = auth.uid() 
    AND p.role IN ('carrier_admin')
  )
);

-- Function to assign driver to carrier
...
-- Function to get carrier's drivers
