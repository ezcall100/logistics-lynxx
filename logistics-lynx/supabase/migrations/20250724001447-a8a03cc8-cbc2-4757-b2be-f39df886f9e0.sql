...
-- Create basic RLS policies for now (can be refined later)
CREATE POLICY "Users can manage EDI configurations"
ON public.edi_configurations
FOR ALL
USING (is_authenticated_user());

CREATE POLICY "Users can manage EDI partnerships"
ON public.edi_partnerships
FOR ALL
USING (is_authenticated_user());

CREATE POLICY "Users can manage EDI templates"
ON public.edi_document_templates
FOR ALL
USING (is_authenticated_user());

CREATE POLICY "Users can view EDI transactions"
ON public.edi_transactions
FOR ALL
USING (is_authenticated_user());

CREATE POLICY "Users can manage failed EDI transactions"
ON public.edi_failed_transactions
FOR ALL
USING (is_authenticated_user());

CREATE POLICY "Users can manage EDI matching rules"
ON public.edi_matching_rules
FOR ALL
USING (is_authenticated_user());

CREATE POLICY "Users can view EDI audit logs"
