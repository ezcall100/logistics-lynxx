// Final System Status Check with proper environment loading
...
async function testMCPApi() {
  ...
  try {
    const response = await makeRequest(`${config.mcpApiUrl}/mcp/system/health`);
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      try {
        ...
        log(`   Found ${agents.data?.length || 0} agents`, 'cyan');
        return true;
      } catch (parseError) {
...
// Main execution
async function main() {
  log('🚀 Starting Final System Status Check', 'bold');
  log('='.repeat(60), 'cyan');
