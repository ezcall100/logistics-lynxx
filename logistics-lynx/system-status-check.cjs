...
// Tests MCP agents, Supabase, n8n, GitHub, OpenAI, and MCP API port
...
// Configuration
const config = {
  mcpApiUrl: 'http://localhost:3001/api',
  supabaseUrl: 'https://imcyiofodlnbomemvqto.supabase.co',
  n8nWebhookUrl: 'https://pixx100.app.n8n.cloud/webhook-test/',
...
async function testMCPApi() {
  log('\n🔧 Testing MCP API (Port 3001)...', 'blue');
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      log(`   Response: ${response.data.substring(0, 100)}...`, 'cyan');
      return true;
    } else {
  ...
  try {
    ...
    const response = await makeRequest(`${config.supabaseUrl}/rest/v1/autonomous_agent_configs?select=count`, {
      headers: {
        ...
        'Authorization': `Bearer ${process.env.VITE_SUPABASE_ANON_KEY || 'test'}`
      }
    });
...
async function testGitHubIntegration() {
  ...
  try {
    // Test GitHub API rate limit (public endpoint)
...
async function testOpenAI() {
  ...
  try {
    if (!process.env.OPENAI_API_KEY) {
      ...
      log('   Set OPENAI_API_KEY environment variable to test', 'cyan');
      return false;
    }
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      try {
        ...
        return true; // Endpoint is working, just parsing issue
      }
    } else {
...
// Main execution
async function main() {
  log('🚀 Starting Comprehensive System Status Check', 'bold');
  log('='.repeat(60), 'cyan');
  ...
  results['MCP API (Port 3001)'] = await testMCPApi();
